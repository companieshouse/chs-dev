...
describe("FinalConfirmationPrompt", () => {
    ...
    it("repeats confirmation 5 times and then rejects with an error", async () => {
        (confirmMock as jest.Mock).mockReturnValue(false);
        (editorMock as jest.Mock).mockReturnValue(yamlRepresentation as never);

        await expect(finalConfirmationPrompt.make(fullServiceSpec)).rejects.toThrowError("Invalid response to question: failed after 5 attempts");
